...
def _euclidean_gcd(P):
    ...
    return lines + [f"The last remainder before 0 is {g} — that is the gcd.",
                    _answer_line(g)]
...
def _lin_dio_solvable(P):
    ...
    if r == 0:
        out.append(f"{c} = {q}·{g} — it is a multiple. Solvable.")
        ans = 1
    else:
...
def _lin_dio_any_pair(P):
    ...
    x, y = x0 * k, y0 * k
    return (lines
            + [f"That reaches {g}, and we need {c} = {k}·{g} — scale the whole "
...
def _mod_inverse_find(P):
    ...
    d = int(gcd(a, n))
    if d != 1:
        return [f"First check gcd({a}, {n}) = {d} — not 1.",
...
def _lin_cong_none(P):
    ...
    return [f"gcd({a}, {n}) = {g}. Whatever x is, {a}·x mod {n} is always a "
            ...
            f"— that remainder is unreachable.",
